...
import { editFacilityWard } from "@/redux/service/inpatient";
import { updateWardStoreAfterPatch } from "@/redux/features/inpatient";

const EditWard = ({open, setOpen, selectedRowData}) => {
  ...
  const initialValues = {
    ward_type: selectedRowData.ward_type || "",
  ...
  const handleEditWard = async (formValue) => {
    try{
        const response = await editFacilityWard(auth, formValue, selectedRowData.id)
