...
import { updateAdmissionDetails } from "@/redux/service/inpatient";
import { fetchHospitalBeds, fetchHospitalWards, updateAdmissionStoreAfterPatch } from "@/redux/features/inpatient";

const EditAdmission = ({open, setOpen, selectedRowData}) => {
  ...
  const initialValues = {
    reason_for_admission: selectedRowData.reason_for_admission || "",
    ward: selectedRowData.ward || "",
    bed: selectedRowData.bed || "",
  };
  ...
  const handleEditAdmission = async (formValue) => {
    console.log("FORM VALUE", formValue);
    setLoading(true);
    try{
        const response = await updateAdmissionDetails(auth, formValue, selectedRowData.id)
        dispatch(updateAdmissionStoreAfterPatch(response))
        ...
        console.log("ERROR UPDATING Admission", error)
        setLoading(false);
        toast.error("Error updatin Admission. Please try again.");
    }
  }
        ...
        >
            <DialogTitle>
              <div className="flex justify-between">
                  <p>{`Name: ${selectedRowData?.patient_first_name} ${selectedRowData?.patient_second_name}`}</p>
                  <p>{`Gender: ${selectedRowData?.patient_gender}`}</p>
                  <p>{`Age: ${selectedRowData?.patient_age}`}</p>
              </div>
            </DialogTitle>
            <DialogContent>
            <Formik
