...
import { createFacilityBed } from "@/redux/service/inpatient";
import { updateBedStoreAfterPost } from "@/redux/features/inpatient";
...
const AddWardBed = () => {
  ...
  const handleAddBed = async (formValue) => {
    try{
        const response = await createFacilityBed(auth, formValue, params.ward_id)
        dispatch(updateBedStoreAfterPost(response))
        handleClose();
        toast.success("Bed created successfully!");

    }catch(error){
        console.log("ERROR CREATING", error)
        toast.error("Error creating bed. Please try again.");
    }
  }
